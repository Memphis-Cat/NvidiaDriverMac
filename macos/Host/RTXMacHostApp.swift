...
private enum DriverConnectionError: LocalizedError {
    ...
    case validationCallFailed(kern_return_t)
    case shortStatus(UInt32)
    ...
    var errorDescription: String? {
        ...
        case .validationCallFailed(let kr):
            return "Driver package validation call failed (\(hexIOReturn(kr)))."
        case .shortStatus(let count):
            return "Driver returned only \(count) validation status values; 8 were required."
        }
    }
}
...
private func findRTXMacService() -> io_service_t {
    // DriverKit services normally register under their IOUserClass name.
    ...
    // Conservative fallback for DriverKit registry layouts that expose only the
    // generic IOUserService class. Verify IOUserClass before returning anything.
...
private func validatePackageWithDriver(_ data: Data) throws -> DriverValidationResult {
    ...
    let callResult: kern_return_t = data.withUnsafeBytes { input in
        output.withUnsafeMutableBufferPointer { outputBuffer in
            IOConnectCallMethod(
                connection,
                UserClientSelector.validatePackage,
                nil,
                0,
                input.baseAddress,
                input.count,
    ...
    guard callResult == KERN_SUCCESS else {
        throw DriverConnectionError.validationCallFailed(callResult)
    }
    guard outputCount == 8 else {
        throw DriverConnectionError.shortStatus(outputCount)
    }
...
private struct RTXMacContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            GroupBox("Validated boot package") {
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        ...
                        .disabled(extensions.validating)
            ...
            Text("Validation is read-only. Selecting a package does not reset the GPU, allocate boot DMA, write BAR0/PRAMIN, execute Falcon firmware, or start GSP-RM.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .padding(24)
