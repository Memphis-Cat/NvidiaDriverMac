...
struct ManifestInputs {
  ...
  std::uint64_t frtsSize{0x100000ull};
  std::uint64_t nonWprHeapSize{};
  std::uint64_t requestedWprHeapSize{};
  std::uint64_t gspFirmwareImageBytes{};
  std::uint64_t gspSignatureBytes{};
  std::uint64_t gspBootloaderBytes{};
  std::uint64_t frtsFwsecImageBytes{};
  std::uint64_t sec2BooterImageBytes{};
...
struct BootManifest {
  ...
  QueueMemoryLayout queues{};
  Radix3Layout radix3{};
  WprLayout wpr{};
  std::vector<AllocationRequirement> allocations;
  // Until the two required pre-boot RPC payloads are serialized, this remains
  // false even though all lower-level memory/Falcon planning is available.
  bool bootstrapRpcPrefillImplemented{};
};
...
struct ResolvedAddresses {
  ...
  std::uint64_t cachedArguments{};
  std::uint64_t libosInitArguments{};
  std::uint64_t wprMetadata{};
  std::uint64_t radix3FirmwareRoot{};
  std::uint64_t firmwareSignature{};
  std::uint64_t gspBootloader{};
  std::uint64_t frtsFwsecImage{};
  std::uint64_t sec2BooterImage{};
};
...
enum class CheckKind : std::uint8_t {
  MmioMaskEqual,
  MmioNonZero,
  SharedMemoryU32Equal,
  HostPreparationRequired,
};

struct Check {
  CheckKind kind{};
  std::uint64_t addressOrOffset{};
  std::uint32_t mask{};
  std::uint32_t value{};
};

enum class BootPhase : std::uint8_t {
  PrefillBootstrapRpcRecords,
  ResetGspForFrts,
  ExecuteFrtsFwsec,
  VerifyWpr2,
  ResetGspForRiscv,
...
struct PhasePlan {
  BootPhase phase{};
  std::vector<falcon::Action> actions;
